...
class Resize(object):
    ...
    def __init__(self, max_size, resize_gt=True):
...
class BaseTransform(object):
    ...
    def __init__(self, transform, input_size, mean=MEANS, std=STD):
        self.augment = Compose([
            ConvertFromInts(),
            Resize(input_size, resize_gt=False),
            BackboneTransform(transform, mean, std, 'BGR')
        ])
...
class SSDAugmentation(object):
    ...
    def __init__(self, transform, input_size, mean=MEANS, std=STD):
        self.augment = Compose([
            ConvertFromInts(),
            ToAbsoluteCoords(),
            enable_if(True, PhotometricDistort()),
            enable_if(True, Expand(mean)),
            enable_if(True, RandomSampleCrop()),
            enable_if(True, RandomMirror()),
            enable_if(False, RandomFlip()),
            enable_if(False, RandomRot90()),
            Resize(input_size),
            enable_if(True, Pad(550, 550, mean)),
            ToPercentCoords(),
            PrepareMasks(16, False),
            BackboneTransform(transform, mean, std, 'BGR')   # 默认resnet。减均值除以标准差。返回HWC且是RGB格式的图片。
        ])
